    ...
    for x, column in enumerate(data):
        ...
        for y, value in enumerate(data[column]):
            ...
            if xstart < x < xend:
                ...
                if value == -1:
                    ax2.add_patch(
                        mpl.patches.Rectangle((x, y), 1, 1, facecolor="None", edgecolor='gold', alpha=0.6, zorder=999))
...
foldx = pd.read_csv('data/raw.parp.ensemble.dat', sep='\t', header=[0, 1], index_col=0, na_values=['-'])
