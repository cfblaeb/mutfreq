...
def heatplot(data, ax1, ax2):
    ...
    for x, column in enumerate(data):
        our_plot_data.append(
            sum(list(data[column]).count(i) for i in our_positive) / sum(list(data[column]).count(i) for i in our_test))
        ...
        for y, value in enumerate(data[column]):
            ...
            if xstart < x < xend:
                ...
                if value == -1:
                    ax2.add_patch(
                        mpl.patches.Rectangle((x, y), 1, 1, facecolor="gold", edgecolor='gold', alpha=0.6, zorder=999))
                ...
                if value in their_positive:
                    ax2.text(x + 0.5, y + 0.5, "x", size=10, color='red', family='monospace', ha='center',
                             va='center', )
...
def enrichdata(control_freq, sorted_freq):
    ...
    enrichment = (sorted_freq / control_freq).loc[~sorted1.index.isin(['^', 'X', '+', '*'])].replace(np.inf, 0).replace(
        np.nan, 0)
    ...
    enrichment /= outlier_border
    return enrichment


def barchart(enricheddata, yerr=None):
    rasmol = {'A': '#C8C8C8', 'R': '#145AFF', 'N': '#00DCDC', 'D': '#E60A0A', 'C': '#E6E600', 'Q': '#00DCDC',
              'E': '#E60A0A', 'G': '#EBEBEB', 'H': '#8282D2', 'I': '#0F820F', 'L': '#0F820F', 'K': '#145AFF',
              'M': '#E6E600', 'F': '#3232AA', 'P': '#DC9682', 'S': '#FA9600', 'T': '#FA9600', 'W': '#B45AB4',
              'Y': '#3232AA', 'V': '#0F820F'}
    rasmol_cmap = mpl.colors.ListedColormap([rasmol[x] for x in enricheddata.index])
    ax = enricheddata.T.plot(kind='bar', stacked=True, colormap=rasmol_cmap, legend=False, yerr=yerr.T)
...
foldx = pd.read_csv('data/raw.parp.ensemble.dat', sep='\t', header=[0, 1], index_col=0)
...
dfcon = pd.concat((enrich1, enrich2))
dfcon[dfcon == 0] = None
dfgroup = dfcon.groupby(dfcon.index)
dfmean = dfgroup.mean()
dferr = dfgroup.std()
barchart(dfmean, dferr)

#pd.concat((enrich1.T, enrich2.T), keys=[1,2]).sortlevel(level=1).plot(kind='bar', stacked=True, legend=False)
...
combinedmap = combinemap(control_freq1, control_freq2, sorted_freq1, sorted_freq2, enrich1, enrich2, foldx,
                         foldx_threshold)
